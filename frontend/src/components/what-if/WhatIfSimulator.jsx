...
export function WhatIfSimulator({ initialBasePath }) {
  const [operation, setOperation] = useState("REPLACE");
  const [target, setTarget] = useState("AI Club");
  const [replacementType, setReplacementType] = useState("Research");
  const [timeLimit, setTimeLimit] = useState(6.0);
